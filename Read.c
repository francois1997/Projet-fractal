...
/*	this function handles all of the different cases that read2 can run into, it also checks wether the stream is the stdin or just a file
 *
 *	it accepts a void pointer which it typecast into a int, that int is the file descriptor it will use
 *
 *	by default it always return NULL but this can be changed
 *
 */
...
void *reading(void* parametre)
{
	...
	if (fileptr->fd == 0)
	{
		//where the string read on the input stream will be put and the length of that string
		char **input = NULL;
		size_t len = 0;
		char *string = NULL;
		getline(input, &len, 0);
